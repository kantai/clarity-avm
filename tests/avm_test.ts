...
Clarinet.test({
    ...
    async fn(chain: Chain, accounts: Map<string, Account>, contracts: Map<string, Contract>) {
        ...
        const instructions = types.list([
            types.tuple({ "op": types.buff(fromHex("61")), "imv": types.some(types.buff(fromHex("00"))) })
        ]);
...
const CLAR_10 = types.buff(fromHex("010000000000000000000000000000000a"));
const CLAR_2 = types.buff(fromHex("0100000000000000000000000000000002"));
const CLAR_PC_2 = types.buff(fromHex("0c00000001037063720100000000000000000000000000000005"));

Clarinet.test({
    name: "Test a longer program",
    async fn(chain: Chain, accounts: Map<string, Account>, contracts: Map<string, Contract>) {
        const alice = accounts.get("wallet_1")!;
        const instructions = [
            types.tuple({ "op": OP_NOP, "imv": types.some(CLAR_10) }),
        ...
        let all_events = execution.receipts[1].events.concat(execution.receipts[2].events);
        console.log(all_events);
        // assertEquals(run_one_out.events, run_one_out.events);
    }
});
